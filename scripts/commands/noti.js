...
module.exports.start = async({ api, event, args, Threads, Users, Cherry }) => {
    ...
    if (args[0] == 'all') {
        ...
        allThread = allThread.filter(item => item.ID != threadID && item.isGroup);
        ...
        if (type == 'message_reply' && messageReply.attachments.length > 0) {
            var data = await Cherry.saveAttachments(messageReply.attachments, __dirname + '/cache/');
            if (data.error) return api.send(`Đã xảy ra lỗi khi lấy attachments. Lỗi: \n${data.error}`);
